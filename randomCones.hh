...
public :
  randomCones(unsigned int nCones = 4, double rParam = 0.4, double etaMax = 2.3, double rho = 0.) :
    nCones_(nCones),
    rParam_(rParam),
    etaMax_(etaMax),
    rho_(rho),
  ...
  double setRho(double rho)   { rho_ = rho; }
  ...
  std::vector<fastjet::PseudoJet> run() {
    ...
    for(unsigned int i = 0; i<nCones_; ++i) {
      ...
      double ptRCSub = ptRC - rho_*TMath::Pi()*rParam_*rParam_;
